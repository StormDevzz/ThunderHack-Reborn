...
public class PacketFly extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (mc.player != null && event.getPacket() instanceof PlayerPositionLookS2CPacket pac) {
            ...
            event.cancel();
            net.minecraft.entity.EntityPosition newChange = new net.minecraft.entity.EntityPosition(
                pac.change().position(), pac.change().deltaMovement(),
                mc.player.getYaw(), mc.player.getPitch()
            );
            mc.player.networkHandler.onPlayerPositionLook(
                net.minecraft.network.packet.s2c.play.PlayerPositionLookS2CPacket.of(
                    pac.teleportId(), newChange, pac.relatives()
                )
            );
