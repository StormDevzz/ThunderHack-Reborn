    ...
    public float getDigSpeed(BlockState state, BlockPos position) {
        ...
        if (digSpeed > 1) {
            ...
            if (slot != -1) {
                ...
                int efficiencyModifier = EnchantmentHelper.getLevel(mc.world.getRegistryManager().getOrThrow(Enchantments.EFFICIENCY.getRegistryRef()).getOrThrow(Enchantments.EFFICIENCY), itemstack);
