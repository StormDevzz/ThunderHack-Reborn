    ...
    public float getDigSpeed(BlockState state, BlockPos position) {
        ...
        if (digSpeed > 1) {
            ...
            if (slot != -1) {
                ...
                int efficiencyModifier = EnchantmentHelper.getLevel(mc.world.getRegistryManager().getOrThrow(net.minecraft.registry.RegistryKeys.ENCHANTMENT).getOrThrow(Enchantments.EFFICIENCY), itemstack);
