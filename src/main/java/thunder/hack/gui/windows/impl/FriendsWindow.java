...
public class FriendsWindow extends WindowBase {
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        for (FriendPlate friendPlate : friendPlates) {
            ...
            boolean online = mc.player != null && mc.player.networkHandler.getListedPlayerListEntries().stream().map(p -> ((PlayerListEntry) p).getProfile().name()).toList().contains(friendPlate.name()) || Managers.TELEMETRY.getOnlinePlayers().contains(friendPlate.name());
